...
st.subheader("Período em análise")

# Navegação por trimestre. Cada trimestre reúne as três competências na mesma tela.
quarter_map={}
for comp in comps:
    mm=int(comp[:2]); yy=int(comp[3:])
    q=(mm-1)//3+1
    key=f"{yy}-T{q}"
    quarter_map.setdefault(key,[]).append(comp)

quarter_keys=list(quarter_map.keys())
quarter_keys.sort(key=lambda x:(int(x[:4]),int(x[-1])))
def quarter_label(key):
    yy=int(key[:4]); q=int(key[-1])
    return f"{q}º trimestre/{yy}"

if "selected_quarter" not in st.session_state:
    initial_comp=st.session_state.get("selected_comp",comps[0])
    yy=int(initial_comp[3:]); q=(int(initial_comp[:2])-1)//3+1
    st.session_state.selected_quarter=f"{yy}-T{q}"

current_q=st.session_state.get("selected_quarter",quarter_keys[0])
if current_q not in quarter_keys:
    current_q=quarter_keys[0]
st.session_state.selected_quarter=current_q

qidx=quarter_keys.index(current_q)
cprev,csel,cnext=st.columns([1,4,1])
with cprev:
    if st.button("◀ Trimestre anterior",disabled=qidx==0,key="quarter_prev"):
        st.session_state.selected_quarter=quarter_keys[qidx-1]
        st.rerun()
with csel:
    ...
    if selected_q!=current_q:
        st.session_state.selected_quarter=selected_q
        st.rerun()
with cnext:
    if st.button("Próximo trimestre ▶",disabled=qidx==len(quarter_keys)-1,key="quarter_next"):
        st.session_state.selected_quarter=quarter_keys[qidx+1]
        st.rerun()

quarter_comps=quarter_map[current_q]
# Mantém current como referência para as abas que ainda operam sobre uma competência.
current=quarter_comps[0]
st.session_state.selected_comp=current

quarter_df=df[df["competencia"].isin(quarter_comps)].copy() if not df.empty else pd.DataFrame()
quarter_inc=inc[inc["competencia"].isin(quarter_comps)].copy() if not inc.empty else pd.DataFrame()

valid_q=quarter_df[quarter_df["validar"]==True] if not quarter_df.empty else pd.DataFrame()
k1,k2,k3,k4,k5=st.columns(5)
k1.metric("Horas trabalhadas",min_to_hhmm(int(quarter_df["horas_min"].sum())) if not quarter_df.empty else "00:00")
k2.metric("Dias trabalhados",int(quarter_df["data"].nunique()) if not quarter_df.empty else 0)
k3.metric("Plantões validados",len(valid_q))
k4.metric("☀️ Diurnos",int((valid_q["turno_analise"]=="☀️ Diurno").sum()) if not valid_q.empty else 0)
k5.metric("🌙 Noturnos",int((valid_q["turno_analise"]=="🌙 Noturno").sum()) if not valid_q.empty else 0)
...
with tabs[0]:
    ...
    st.caption(
        "🟢 completo e validado automaticamente. "
        "🟡 horário para conferir. 🔴 falta de entrada/saída. "
        "O Turno e a coluna Validar podem ser alterados pelo usuário."
    )
    st.markdown(f"### {quarter_label(current_q)}")

    # As três competências ficam na mesma tela, em sequência cronológica.
    for comp in quarter_comps:
        mdf=df[df["competencia"]==comp].copy() if not df.empty else pd.DataFrame()
        mi=inc[inc["competencia"]==comp].copy() if not inc.empty else pd.DataFrame()
        pdfs=st.session_state.get("periodos",pd.DataFrame())
        mp=pdfs[pdfs["competencia"]==comp].copy() if not pdfs.empty else pd.DataFrame()

        st.markdown(f"#### Competência {comp}")

        if mdf.empty:
            st.info(
                "Ficha localizada, mas não foram encontradas batidas para formar eventos."
            )
            if not mp.empty:
                for _,pr in mp.iterrows():
                    st.caption(
                        f"📄 {pr['pdf']} • página {int(pr['pagina'])} • formato {pr['modelo']}"
                    )
        else:
            view=mdf.copy()
            view["status_visual"]=view["status"].map(status_icon)

            # Marca especificamente o campo que falta/está problemático.
            err_by_date={}
            if not mi.empty:
                for _,ir in mi.iterrows():
                    msg=str(ir["problema"])
                    err_by_date.setdefault(ir["data"],[]).append(msg)

            view["entrada_display"]=view["entrada"]
            view["saida_display"]=view["saida"]
            view["erro_campo"]=""
            for ridx,r in view.iterrows():
                msgs=err_by_date.get(r["data"],[])
                if msgs:
                    msg=" ".join(msgs).lower()
                    if "apenas um horário" in msg or "entrada" in msg:
                        if not r["entrada"]:
                            view.loc[ridx,"entrada_display"]="🔴 —"
                    if "saída" in msg:
                        if not r["saida"]:
                            view.loc[ridx,"saida_display"]="🔴 —"
                    view.loc[ridx,"erro_campo"]="🔴 " + " / ".join(msgs)

            view["intervalos"]=view["intervalos"].replace("", "—")
            view["turno_analise"]=view["turno_analise"].replace("", "—")
            view["plantao_sugerido_texto"]=np.where(view["plantao_sugerido"],"SIM","NÃO")

            view_cols=[
                "status_visual","validar","data","entrada_display","saida_display",
                "intervalos","horas","horas_documento","turno_analise",
                "plantao_sugerido_texto","erro_campo"
            ]

            edited=st.data_editor(
                view[view_cols],
                key=f"editor_auditoria_{comp.replace('/','_')}",
                hide_index=True,
                ...
                column_config={
                    ...
                    "entrada_display":st.column_config.TextColumn("Entrada"),
                    "saida_display":st.column_config.TextColumn("Saída"),
                    ...
                    "turno_analise":st.column_config.SelectboxColumn(
                        "Turno",options=["☀️ Diurno","🌙 Noturno"],width="small"
                    ),
                    "plantao_sugerido_texto":st.column_config.TextColumn(
                        "Plantão sugerido",width="small"
                    ),
                    "erro_campo":st.column_config.TextColumn(
                        "Inconsistência",width="medium"
                    ),
                }
            )
            ...
            for ridx,val in edited["validar"].items():
                new_val=bool(val)
                if new_val != bool(df.loc[ridx,"validar"]):
        ...
        # Inconsistências que não têm evento formado entram na sequência da
        # auditoria como linhas documentais somente, sem checkbox/edição.
        if not mi.empty:
            st.caption("Ocorrências documentais nesta competência")
            inc_seq=[]
            for _,ir in mi.sort_values("data").iterrows():
                inc_seq.append({
                    "Status":status_icon(ir["status"]),
                    "Data":ir["data"],
                    "Entrada":"🔴 —" if "entrada" in str(ir["problema"]).lower() or "apenas um horário" in str(ir["problema"]).lower() else "—",
                    "Saída":"🔴 —" if "saída" in str(ir["problema"]).lower() else "—",
                    "Campo com erro":"🔴",
                    "Problema":ir["problema"],
                    "Página":int(ir["pagina"])
                })
            st.dataframe(
                pd.DataFrame(inc_seq),
                hide_index=True,use_container_width=True,
                column_config={
                    "Data":st.column_config.DateColumn("Data",format="DD/MM/YYYY"),
                    "Status":st.column_config.TextColumn("Status",width="small"),
                    "Campo com erro":st.column_config.TextColumn("Campo",width="small"),
                }
            )

    # Evidência da competência selecionada dentro do trimestre.
    st.markdown("### Ver evidência do registro")
    qev=quarter_df
    if not qev.empty:
        opts=[
            f"{i} • {r['competencia']} • {r['data'].strftime('%d/%m/%Y')} • {r['horas']} • {r['id_evento']}"
            for i,r in qev.iterrows()
        ]
        sel=st.selectbox("Selecione um registro",opts,key=f"aud_evidence_q_{current_q}")
...
with tabs[1]:
    ...
    month_inc=inc[inc["competencia"].isin(quarter_comps)].copy() if not inc.empty else pd.DataFrame()
