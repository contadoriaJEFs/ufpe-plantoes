...
with tabs[0]:
    ...
    if month_df.empty:
        ...
        view_cols=[
            "validar","competencia","data","entrada","saida","intervalos",
            "horas","horas_documento","turno_analise","plantao_sugerido","status_visual"
        ]
        ...
        edited=st.data_editor(
            ...
            disabled=[c for c in view_cols if c!="validar"],
            column_config={
                ...
                "plantao_sugerido":st.column_config.CheckboxColumn("Plantão sugerido",disabled=True),
        ...
        for ridx,val in edited["validar"].items():
            ...
            if new_val != old_val:
                ...
                if new_val != bool(df.loc[ridx,"plantao_sugerido"]):
        ...
        if changed:
            st.session_state.records=df
            # A contagem de diurnos/noturnos passa a depender exclusivamente
            # desta classificação analisada pelo usuário.
    ...
    q4.metric("☀️ Diurnos",int((valid["turno"]=="☀️ Diurno").sum()))
    q5.metric("🌙 Noturnos",int((valid["turno"]=="🌙 Noturno").sum()))
